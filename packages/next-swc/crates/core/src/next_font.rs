...
pub type Config = HashMap<String, FontData>;
...
pub struct FontData {
    family: String,
    // hash: String, //   staticPath: string {assetPrefix/}_next/static/fonts/{file}
    path: String,
}
...
impl Fold for NextFont {
        ...
        if let Callee::Expr(i) = &expr.callee {
            if let Expr::Ident(identifier) = &**i {
                if &identifier.sym == "preloadFont" {
                    ...
                    let font_file = match self.fonts.get(font_name) {
                        Some(file) => file,
                        None => panic!("AHH"),
                    };
                    ...
                    expr.args[0] = ExprOrSpread {
                        ...
                        expr: Box::new(Expr::Lit(Lit::Str(Str {
                            // value: JsWord::from(format!("/_next/static/fonts/{}",
                            // font_file.path)),
    ...
    // &*expr.args[0].expr {                 self.dynamically_imported_specifier
    ...
    //                             .struct_span_err(
    //                                 identifier.span,
    //                                 "next/dynamic requires at least one
    // argument",                             )
    //                             .emit()
    //                     });
    //                     return expr;
    ...
    //                 expr.args[0].expr =
    // expr.args[0].expr.clone().fold_with(self);
    ...
    //                 // loadableGenerated: {
    //                 //   webpack: () =>
    // [require.resolveWeak('../components/hello')],                 let
    // generated = Box::new(Expr::Object(ObjectLit {                     span:
    // DUMMY_SP,                     props: if self.is_development ||
    // self.is_server {
    // vec![PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
    //                             key: PropName::Ident(Ident::new("modules".into(),
    // DUMMY_SP)),                             value:
    // Box::new(Expr::Array(ArrayLit {                                 elems:
    // vec![Some(ExprOrSpread {                                     expr:
    ...
    //
    // vec![PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
    //                             key: PropName::Ident(Ident::new("webpack".into(),
    // DUMMY_SP)),                             value:
    // Box::new(Expr::Arrow(ArrowExpr {                                 params:
    // vec![],                                 body:
    // BlockStmtOrExpr::Expr(Box::new(Expr::Array(ArrayLit {
    ...
    // vec![PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
    //                         key:
    // PropName::Ident(Ident::new("loadableGenerated".into(), DUMMY_SP)),
    //                         value: generated,
    //                     })))];
    ...
    //                             if let Some(KeyValueProp { key, value }) = match
    // prop {                                 PropOrSpread::Prop(prop) => match
    // &**prop {
    ...
    //                     }
    //                 }
    //                 // Don't need to strip the `loader` argument if suspense is
    // true                 // See https://github.com/vercel/next.js/issues/36636 for background
