...
const googleFontsFixtures = require.resolve('./google-font-mocked-responses.js')

// _error
describe('@next/font/google loader', () => {
  let next: NextInstance

  const setup = async (fixture: string) => {
  ...
  test('Add preconnect', async () => {
    await setup('global-font')
  ...
  test('Add preload', async () => {
    await setup('global-font-preload')
    const html = await renderViaHTTP(next.url, '/')
    const $ = cheerio.load(html)
    ...
    expect($('link[as="font"]').get(0).attribs).toEqual({
      as: 'font',
      crossorigin: 'anonymous',
      href: '/_next/static/fonts/086deb0378c2136e.p.woff2',
      rel: 'preload',
      type: 'font/woff2',
    })
  })
